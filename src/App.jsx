...
function InvoiceForm({ invoice, clients, setClients, settings, onCancel, onSave }) {
  ...
  function addItem() {
    setForm((f) => ({ ...f, items: [...f.items, { id: uid(), desc: "", qty: 1, rate: 0 }] }));
  }
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="rounded-lg p-6 mb-5" style={{ background: C.panel, border: `1px solid ${C.line}` }}>
        ...
        <div className="grid grid-cols-[1fr_70px_100px_100px_32px] gap-2 text-xs font-medium mb-2" style={{ color: C.textMuted }}>
          <div>Description</div><div>Qty</div><div>Rate (₹)</div><div className="text-right">Amount</div><div></div>
        </div>
        {form.items.map((it) => (
          <div key={it.id} className="grid grid-cols-[1fr_70px_100px_100px_32px] gap-2 mb-2 items-center">
            <input
